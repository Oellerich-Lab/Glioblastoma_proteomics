...
harmonize_df_parametric <- readRDS("20230106_harmonized.rds")
```
...
colnames(cluster_df) <- c("prot_id", "value")
...
ht <- Heatmap(as.matrix(dt),
              col = color_fun_glio,
              column_split = data.frame(df_meta$value),
              row_split = 5,
              column_title = " ",
              row_title = " ",
              border = F,
...
gsva_out_prot <- gsva.out %>% 
  ...
  left_join(df_meta %>% dplyr::select(prot_id, value), by= "prot_id") %>%
...
#save as csv
write.csv(df_norm_uncorrected, "20230106_gioblastoma_proteome_norm_noBatchCorr.csv")
...
# save as csv
write.csv(df_norm_batch_corrected, "20230106_gioblastoma_proteome_norm_withBatchCorr.csv")


#export meta data 
df_exp <- df_meta %>%
  dplyr::select(label, tmt_set_name_2, patient, proteomic_id, idh_wt, prot_id)

#save as csv
write.csv(df_exp, "20230106_glioblastoma_meta_data.csv")
```
